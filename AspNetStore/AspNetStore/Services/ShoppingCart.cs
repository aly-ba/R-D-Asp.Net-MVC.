...
namespace AspNetStore.Services
{
    public class ShoppingCart
    {
        ...
        public async Task AddAsync(int productId)
        {
            ...
            if (product == null)
            {
                //TODO: throw exception  or do something else
                return;
            }
            var cartItem = await _db.CartItems.SingleOrDefaultAsync(
                c => c.ProductId == product.ProductId && c.CartId == _cartId);
        ...
        {
            var cartItem = await _db.CartItems.SingleOrDefaultAsync(
                c => c.ProductId == productId && c.CartId == _cartId);
        ...
        public static decimal CalculateCart(IEnumerable<CartItem> items)
        {
